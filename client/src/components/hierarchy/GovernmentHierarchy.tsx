...
interface TerritorialArea {
  area_id: number
  area_name: string
  area_type: 'region' | 'province' | 'commune'
...
interface Official {
  ...
  territorial_area?: TerritorialArea
}
...
const GovernmentHierarchy: React.FC = () => {
  const [hierarchyData, setHierarchyData] = useState<{
    grades: OfficialGrade[]
    officials: Official[]
    institutions: Institution[]
    territorialAreas: TerritorialArea[]
  }>({
    grades: [],
    officials: [],
    institutions: [],
  ...
  const [expandedNodes, setExpandedNodes] = useState<Set<number>>(new Set([1, 2, 3])) // Start with some expanded
  ...
  const loadHierarchyData = async () => {
    try {
      ...
      const mockGrades: OfficialGrade[] = [
        {
          grade_id: 1,
          grade_name: 'Monarque',
          level: 1,
      ...
      const mockTerritorialAreas: TerritorialArea[] = [
  ...
  const renderTreeStructure = () => {
    ...
    const renderTreeNode = (node: any, depth: number = 0) => {
      ...
      return (
        <div key={node.id} style={{ position: 'relative', marginLeft: `${depth * 40}px` }}>
          ...
          <div style={{
            ...
            >
              ...
              <div style={{ paddingLeft: hasChildren ? '40px' : '0px' }}>
                <h3 style={{
                  fontSize: '18px',
                  fontWeight: '600',
                  color: '#2d3436',
                ...
                <div style={{
                  ...
                  Niveau {node.level} - {node.level === 1 ? 'Monarque' : 
    ...
    return (
      <div style={{ position: 'relative' }}>
        <h2 style={{
          ...
          Hiérarchie Gouvernementale du Maroc
        </h2>
    ...
    return (
      <div style={styles.container} className="text-white">
        <div style={styles.loading}>
          Chargement de la hiérarchie gouvernementale...
        </div>
      </div>
    )
  }
    ...
    <div style={styles.container} className="text-white">
      ...
      <div style={styles.header} className="backdrop-blur-sm bg-white/5 border border-white/10">
        ...
        <p style={styles.subtitle} className="text-white/80">
          Structure administrative du Maroc du niveau national au niveau local
        </p>
      </div>
